...
```sh
deploy proxy restart 
```
restart the proxy (useful for forcing label re-detection when services are not reachable)
...
```sh
deploy svc restart --name <service> --remote <host> --username <user> --key <ssh_key>
```

- Restarts the service container. Useful for applying configuration changes without full redeploy.
...
When services report as running but are not accessible via the proxy, run diagnostics to identify connectivity issues.

```sh
deploy diagnostic run --remote <host>
```

This command:
- Checks if the proxy container is running
- Lists all services with Caddy labels
- Verifies each service is reachable from the proxy
- Reports container status, IP address, and Caddy routing configuration

With `--fix` flag to attempt automatic remediation:

```sh
deploy diagnostic run --remote <host> --fix
```

The fix option restarts the proxy container to force re-detection of service labels.
